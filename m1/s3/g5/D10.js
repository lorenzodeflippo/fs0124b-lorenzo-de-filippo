...
const stringa = "Funzioni";
const PrimoCarattere = deleteOne(stringa, true);
...
function isThisAnEmail(email) {
  ...
  return atIndex > 0 && dotIndex > atIndex + 1 && hasValidDomain;
}
const email = "test@example.com";
const isValidEmail = isThisAnEmail(email);
...
function isTodayMyBirthday() {
  ...
  const birthDate = new Date(today.getFullYear(), 2, 21);
  const isSameMonth = today.getMonth() === birthDate.getMonth();
  const isSameDay = today.getDate() === birthDate.getDate();
...
const updateObject = deleteProp(oggetto, "cognome");
...
const result2 = searchByTitle(searchTerm);
...
const tdElements = selectTdElements();
...
printTdText();
console.log(tdElements.textContent);
